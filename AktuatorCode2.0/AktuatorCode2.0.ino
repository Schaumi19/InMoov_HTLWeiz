...
void setup(){
    ...
    for (int i = 0; i < 4; i++){
        Motors[i] = new Motor(used[i],min_angle[i], max_angle[i], min_pot[i], max_pot[i], reversed_output[i], reversed_input[i], ContinuousMovement[i]);
...
void receiveEvent(byte aktuatorID, byte angle){
  if (aktuatorID == 0){    //Set all Aktuators to the same Value
    for (byte i = 0; i < 4; i++){
      ...
      Motors[aktuatorID-1] = Motors[aktuatorID-1].SetAngle(angle);
  }
}
